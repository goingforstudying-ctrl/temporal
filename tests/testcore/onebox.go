...
func (c *TemporalImpl) installHostTestHooks(
	...
	case primitives.HistoryService:
		addCleanup(testhooks.Set(
			c.testHooks,
			testhooks.HistoryChasmComponentsCreated,
			func(refs testhooks.HistoryChasmComponents) {
				c.chasmEngine = refs.Engine
				c.chasmVisibilityMgr = refs.VisibilityManager
				c.chasmRegistry = refs.Registry
			},
			testhooks.GlobalScope,
		))
		addCleanup(testhooks.Set(
