...
func ServiceRefsHook(params serviceRefsHookParams) {
	if hook, ok := testhooks.Get(params.TestHooks, testhooks.HistoryChasmComponentsCreated, testhooks.GlobalScope); ok {
		hook(testhooks.HistoryChasmComponents{
			Engine:            params.ChasmEngine,
			VisibilityManager: params.ChasmVisibilityManager,
			Registry:          params.ChasmRegistry,
		})
	}
}
